...
export default function CommandCenter() {
  ...
  useEffect(() => {
    fetchMissions();
    // Simple Weather Simulation (Lagos Default)
    const mockWeather = ["31°C SUNNY", "29°C HUMID", "33°C CLEAR"];
  ...
  const deleteMission = async (id: string) => {
    if (!supabase || !confirm("PERMANENTLY DELETE MISSION?")) return;
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-cyan-500 p-6 font-mono selection:bg-cyan-500 selection:text-black">
      ...
      <header className="mb-10 flex justify-between items-start border-b border-cyan-900/30 pb-6">
        <div>
          ...
          <div className="flex gap-4 mt-1 text-[10px] font-bold opacity-60 italic">
            <span className="flex items-center gap-1"><CloudSun size={12} /> {weather}</span>
            <span className="flex items-center gap-1"><Calendar size={12} /> {new Date().toLocaleDateString()}</span>
            <span>{VERSION}</span>
          </div>
        </div>
        <RefreshCw size={18} className={isSyncing ? "animate-spin" : "opacity-20 cursor-pointer"} onClick={fetchMissions} />
      </header>

      {/* INPUT FORM (RETURN KEY ACTIVE) */}
      <form onSubmit={addMission} className="mb-12 relative group">
        <input
          ...
          className="w-full bg-transparent border-b-2 border-cyan-900 p-4 text-sm focus:outline-none focus:border-cyan-400 transition-all uppercase placeholder:opacity-20"
        />
        <button type="submit" className="absolute right-4 top-4 opacity-40 group-focus-within:opacity-100"><Plus size={20} /></button>
      </form>
      ...
      <div className="grid gap-4">
        {missions.map((m) => (
          ...
          >
            ...
            >
              <div>
                <p className="text-[9px] opacity-40 tracking-widest">{m.category}</p>
                <h2 className="text-sm font-bold text-cyan-100">{m.title}</h2>
              </div>
              <div className="text-[10px] opacity-40">
                {m.tasks.filter(t => t.completed).length}/{m.tasks.length} SYNCED
              </div>
            </div>

            {/* EXPANDED EDIT/TASK PANEL */}
            {expandedId === m.id && (
              <div className="p-5 pt-0 border-t border-cyan-900/30 animate-in fade-in slide-in-from-top-2">

                {/* RENAME SECTION */}
                <div className="mt-4 mb-6 flex gap-2">
                  <input
                    className="flex-1 bg-cyan-900/20 border border-cyan-800 p-2 text-xs text-white uppercase outline-none"
                    value={editTitle}
                    onChange={(e) => setEditTitle(e.target.value)}
                  />
                  <button onClick={() => updateMissionTitle(m.id)} className="bg-cyan-600 text-black p-2"><Check size={16} /></button>
                  <button onClick={() => deleteMission(m.id)} className="bg-red-900/20 text-red-500 p-2"><Trash2 size={16} /></button>
                </div>
                ...
                <div className="space-y-2 mb-6">
                  {m.tasks.map(t => (
                    <div key={t.id} onClick={() => toggleTask(m.id, t.id)} className="flex items-center gap-3 text-xs cursor-pointer group">
                      <div className={`w-4 h-4 border flex items-center justify-center ${t.completed ? 'bg-cyan-500 border-cyan-500 text-black' : 'border-cyan-800'}`}>
                ...
                {/* ADD SUB-TASK (RETURN KEY ACTIVE) */}
                <form onSubmit={(e) => addTask(e, m.id)} className="flex border-t border-cyan-900/30 pt-4 gap-2">
                  <input
                    placeholder="ADD SUB-TASK..."
                    className="flex-1 bg-transparent text-[10px] uppercase focus:outline-none"
                    value={newTaskText}
                    onChange={(e) => setNewTaskText(e.target.value)}
                  />
                  <button type="submit" className="text-cyan-400 opacity-50 hover:opacity-100"><Plus size={14} /></button>
                </form>
              </div>
            )}
          </div>
      ...
      <footer className="mt-20 py-10 opacity-20 text-[8px] flex justify-between border-t border-cyan-900/20 tracking-[0.5em]">
        <div className="flex items-center gap-2"><Lock size={10} /> ENCRYPTION_ACTIVE</div>
        <span>LAGOS_STATION_LINKED</span>
      </footer>
    </main>
  );
}